...
Currently, we're preferring the "Code Only" approach without any exception in our projects. For now, we're using [SnapKit/Masonry][snapkit-masonry] for Objective-C and [SnapKit][snapKit] as a 3rd party Auto Layout DSL. Our goal is switching to the Cocoa Touch's native SDK for Auto Layout. Please also check [Auto Layout Libraries](#auto-layout-libraries) section for more detail about the Auto Layout.
...
Currently, we didn't use the Carthage in any project yet. And, We don't have any logical reason for it.
...
We don't use the groups, we prefer to use the physical directories instead.
...
As we say before, we prefer to use SnapKit.
...
* [Clean-Architecture-for-iOS][clean-architecture]
...
[clean-architecture]: http://basememara.com/swift-clean-architecture/

Currently, we're using Apple's default MVC approach. Our goal is switching to the Clean Architecture when possible.
...
Keep your models immutable, and use them to translate the remote API's semantics and types to your app. For Objective-C projects, Github's [Mantle](mantle) or [JSONModel](jsonmodel) is a good choice. In Swift, you can use structs instead of classes to ensure immutability, and use a parsing library such as [SwiftyJSON][swiftyjson] or [Argo][argo] to do the JSON-to-model mapping.
...
Currently, we're using Vector Images. Requesting Vector Images from Designer is your Developer's responsibility.
...
- Please don't hesitate to send your suggestions 🤗
